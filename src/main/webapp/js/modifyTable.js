$(function () {
    ...
    function getData() {
        if (sessionStorage.modifyTableList == undefined) {
            arr = [];
        } else {
    ...
    function saveData(data, rows) {
        ...
        sessionStorage.modifyTableList = JSON.stringify(data);
    }
    ...
    function add() {
        ...
        $.each(data, function (i, v) {
            $("<tr>").attr("index", i).html("<td contenteditable='true' data-role='orderNumber'>" + (v.orderNumber=i+1) + "</td>"
                ...
                "<td data-role='money'><input id='money" + i + "' value='"+(v.purchase*v.amount)+"' style='width: 100px;margin-left: 0px' /></td>" +
...
function tableListData() {
    ...
    $('#tableListSpan').html("<input type='hidden' name='tableList' value='"
        + JSON.stringify(sessionStorage.modifyTableList) + "' />");
}
